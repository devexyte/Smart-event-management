  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {userTeam && (
          <span className="text-xs px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 font-mono self-start">
            Team: <span className="text-indigo-400 font-bold">{userTeam.name}</span>
          </span>
        )}
      </div>
      ...
      <div className="glass-panel p-5 rounded-3xl">
        <div className="grid grid-cols-4 gap-2 text-center text-xs">
          <div className="p-3 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 font-semibold">
      ...
      <form onSubmit={handleSubmit} className="glass-panel p-6 sm:p-8 rounded-3xl space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
            />
          </div>
        ...
        {/* Track Selection */}
        <div>
          <label className="text-xs font-semibold text-slate-300 block mb-1.5">Track / Category</label>
          <select
            value={track}
            onChange={(e) => setTrack(e.target.value)}
            className="w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-indigo-500"
          >
        ...
        <div>
          <label className="text-xs font-semibold text-slate-300 block mb-1.5">
            Problem Statement Being Solved <span className="text-rose-400">*</span>
          </label>
          <textarea
            rows={3}
            required
            ...
            className="w-full px-4 py-2.5 bg-slate-950/60 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 leading-relaxed"
          />
        </div>
        ...
        <div>
          <label className="text-xs font-semibold text-slate-300 block mb-1.5">
            Proposed Solution & Architecture <span className="text-rose-400">*</span>
          </label>
          <textarea
            rows={3}
            required
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-800">
          <div className="text-xs text-slate-400">
            {existingSub ? (
              <span className="text-emerald-400 font-medium">✓ Project already submitted. You can edit anytime.</span>
            ) : (
          ...
          <div className="flex items-center gap-3">
            <button
              type="submit"
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white text-xs font-bold shadow-lg shadow-indigo-600/30 transition-all hover:scale-[1.02] flex items-center gap-2"
            >
