  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-2 max-w-sm w-full pointer-events-none">
      {toasts.map((toast) => {
        ...
        let border = 'border-emerald-500/30 bg-slate-900/95';
        ...
        if (toast.type === 'warning') {
          ...
          border = 'border-indigo-500/30 bg-slate-900/95';
        ...
        return (
          <div
            key={toast.id}
            className={`pointer-events-auto flex items-start gap-3 p-4 rounded-xl border ${border} shadow-2xl backdrop-blur-xl transition-all duration-300 animate-in slide-in-from-bottom-5`}
          >
            {icon}
