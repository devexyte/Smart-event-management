    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#131d31] border border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-sm transition-colors flex items-center gap-2 self-start sm:self-auto shrink-0"
        >
      ...
      {/* Control Filters */}
      <div className="bg-[#131d31] border border-slate-800 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
        <div className="space-y-1.5">
          <label className="font-semibold text-slate-200">Select Event Scope</label>
          <select
        ...
        <div className="space-y-1.5">
          <label className="font-semibold text-slate-200">Select Report Type</label>
          <select
            value={reportType}
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-200">
                {currentRegistrations.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-800/40">
                    ...
                    <td className="py-2.5 px-3 truncate max-w-xs">{r.eventTitle}</td>
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-200">
                {currentRegistrations.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-800/40">
                    <td className="py-2.5 px-3 font-semibold">{r.studentName}</td>
                    <td className="py-2.5 px-3 truncate max-w-xs">{r.eventTitle}</td>
                    <td className="py-2.5 px-3 font-mono text-[11px] text-blue-300">{r.qrToken}</td>
                    <td className="py-2.5 px-3">
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-200">
                {currentFeedbacks.map((f) => (
                  <tr key={f.id} className="hover:bg-slate-800/40">
                    <td className="py-2.5 px-3 font-semibold">{f.studentName}</td>
                    <td className="py-2.5 px-3 truncate max-w-xs">{f.eventTitle}</td>
                    <td className="py-2.5 px-3 font-bold text-amber-400">{f.rating} ★</td>
                    <td className="py-2.5 px-3 text-slate-300 max-w-md">{f.reviewText}</td>
              ...
              <tbody className="divide-y divide-slate-800/60 text-slate-200">
                {currentCertificates.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-800/40">
                    <td className="py-2.5 px-3 font-mono text-amber-400 font-bold">{c.certificateNumber}</td>
                    <td className="py-2.5 px-3 font-semibold">{c.studentName}</td>
                    <td className="py-2.5 px-3 truncate max-w-xs">{c.eventTitle}</td>
                    <td className="py-2.5 px-3 font-mono text-slate-400 text-[11px]">{c.verificationCode}</td>
                    <td className="py-2.5 px-3 text-slate-300">{c.completionDate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
